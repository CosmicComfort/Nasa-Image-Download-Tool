#!/usr/bin/env python3
"""
NASA Image Download Tool - optimized + adaptive throttling

Improvements on previous version:
- Concurrent downloads with ThreadPoolExecutor and per-thread sessions (connection pooling + retries).
- Adaptive throttling: downloader reduces/increases concurrency and inserts cooldowns when it detects
  many 429 (Too Many Requests) or high failure rates. This helps be fast while staying polite.
- Configurable min/max workers, --adaptive to toggle the adaptive controller.
- Larger chunk size (64 KiB) by default.
- Page-level batching: downloads collected per page are executed concurrently, then controller evaluates metrics.
- Startup logging includes resolved output folder and concurrency settings.

Usage:
- Interactive: python nasa_downloader.py
- Fast CLI: python nasa_downloader.py --query moon --no-prompt --workers 8 --limit 50
- Adaptive enabled by default; disable with --no-adaptive
"""
...
class AdaptiveController:
    """
    Thread-safe controller for adaptive concurrency.
    - Collects counters for successes/failures/throttles (429).
    - After a batch (page) completes, evaluate and return new worker count and optional cooldown seconds.
    - Counters reset after evaluation.
    """
    ...
    def evaluate_and_adjust(self, current_workers: int) -> Tuple[int, float]:
        """
        Decide new worker count and cooldown seconds.
        Returns (new_workers, cooldown_seconds).
        """
        with self.lock:
        ...
        # If many throttles -> reduce aggressively and cooldown.
        if thr_rate > 0.08:  # >8% 429s in the batch
            new_workers = max(self.min_workers, int(current_workers * 0.5))
            cooldown = min(60, 5 + int(thr_rate * 200))  # scale up cooldown
        ...
        # If failure rate high -> reduce cautiously
        if fail_rate > 0.25:  # >25% failures
        ...
        # If success rate very high, try to increase a bit
...
def _download_task(url: str, dest: Path, controller: Optional[AdaptiveController], chunk_size: int = 65536, retries: int = 3) -> bool:
    """
    Worker download task: uses per-thread session, distinguishes 429 throttles for adaptive logic.
    Reports 'success'/'fail'/'throttle' to controller if provided.
    """
    session = get_thread_session(pool_maxsize=10, retries=max(1, retries), backoff_factor=0.5)
    for attempt in range(1, retries + 1):
        try:
            with session.get(url, stream=True, timeout=30) as r:
                # Detect 429 before raising
            ...
            # If HTTP status was 4xx/5xx other than 429, treated as fail (will be retried per loop)
...
def download_urls_concurrent(url_dest_pairs: List[Tuple[str, Path]], workers: int = 6, controller: Optional[AdaptiveController] = None) -> int:
    """
    Download a list of (url, dest) pairs concurrently. Returns count of successful downloads.
    Reports results to controller if provided.
    """
    success_count = 0
    if not url_dest_pairs:
        return 0
    total = len(url_dest_pairs)
    # Submit tasks and track with as_completed + tqdm
...
def crawl_nasa_images(
    ...
    while True:
        ...
        if download_images and page_url_dest_pairs:
            # Download concurrently and update counters via controller
            downloaded = download_urls_concurrent(page_url_dest_pairs, workers=current_workers, controller=controller)
            total_downloaded += downloaded

        # Evaluate adaptive controller (if enabled) and possibly adjust workers and cooldown
...
def main(argv=None):
    try:
        parser = argparse.ArgumentParser(description="NASA Image Download Tool (optimized + adaptive)")
        ...
        # Kick off crawl with optimized concurrency and adaptive controller
        crawl_nasa_images(
            query=query,

...
- Command-deck themed interactive prompts
- Procedural galaxy "zoom-out" startup animation (math-based)
- No new external dependencies: requires requests and tqdm

Usage:
    python nasa_downloader.py            # interactive (shows galaxy intro)
...
def enable_ansi_on_windows():
    """Try to enable ANSI escape sequence processing on Windows consoles (best-effort)."""
    if os.name != "nt":
        return
    try:
        import ctypes
...
# ASCII intensity palette (low to high)
PALETTE = [" ", ".", ":", "-", "=", "+", "*", "O", "0", "@", "#"]


def map_intensity_to_char(val: float) -> str:
    """Map a 0..1 intensity to a character from PALETTE."""
...
def generate_galaxy_particles(n: int = 1200, arms: int = 4, randomness: float = 0.6):
    """
    Generate particle coordinates for a spiral galaxy in normalized galaxy space.
    Returns list of tuples (x, y, intensity) where x,y are floats (galaxy coordinates).
    """
    particles = []
    max_theta = 6.0 * math.pi  # how long the arms are
    for i in range(n):
        # radial parameter t in [0,1]
        t = random.random()
        # theta along the arm
        theta = t * max_theta
        arm = i % arms
        arm_offset = (arm / arms) * (2 * math.pi)
        # logarithmic/linear spiral behavior
        # base radius grows with theta
        r = 0.5 * (theta / max_theta) ** 0.8 * 3.0  # scaled radial extent
        # spin influence to create a graceful spiral
        spin = 1.5
        ...
        x = r * math.cos(angle)
        y = r * math.sin(angle)
        # intensity stronger near center and for some randomness
...
def render_frame(particles: List[Tuple[float, float, float]], cols: int, rows: int, scale: float, cx: float = 0.0, cy: float = 0.0):
    """
    Render a frame into a string with given terminal columns/rows.
    scale maps galaxy coordinates -> terminal cells.
    cx, cy are offsets (center).
    """
    # build intensity buffer
    W, H = cols, rows
    ...
    for (x, y, intensity) in particles:
        # map galaxy coords -> terminal pixel position
        px = int(cx_term + (x - cx) * scale)
        py = int(cy_term + (y - cy) * scale * 0.5)  # squash to approximate terminal aspect
        if 0 <= px < W and 0 <= py < H:
            # accumulate intensity (simple additive)
            buffer[py][px] = min(1.0, buffer[py][px] + intensity)
            # gentle spread to neighbors for smoother look
            for dx, dy, w in ((1, 0, 0.25), (-1, 0, 0.25), (0, 1, 0.25), (0, -1, 0.25)):
                nx, ny = px + dx, py + dy
                if 0 <= nx < W and 0 <= ny < H:
                    buffer[ny][nx] = min(1.0, buffer[ny][nx] + intensity * w)
    # convert intensity to chars
    lines = []
    for row in buffer:
...
def galaxy_zoom_animation(duration: float = 3.5, frames: int = 40):
    """
    Procedural galaxy formation and zoom-out animation.
    duration: total seconds
    frames: number of frames to render
    """
    enable_ansi_on_windows()
    term_w, term_h = get_terminal_size()
    # allocate a drawing region slightly smaller than terminal to allow header/footer
    cols = min(120, term_w)
    rows = min(40, term_h - 6)  # leave room for header/footer
    if rows < 8 or cols < 40:
        # small terminal -> skip fancy animation
        return

    hide_cursor()
    try:
        # create particle set (galaxy coords)
        ...
        for i in range(frames):
            ...
            # phases:
            # 0..0.15 : explosion & assembly (zoomed in)
            # 0.15..0.85 : formation -> spiral become clearer
            # 0.85..1.0 : zoom out to show whole galaxy
            if t < 0.15:
                # initial burst: heavy zoomed-in scale, jittered positions
                interp = t / 0.15
            ...
            # slight camera wobble for cinematic effect
            wobble = math.sin(t * 6.28 * 1.4) * 0.02
            cx += wobble * (1 - t) * 1.5
            cy += math.cos(t * 3.14 * 1.7) * 0.02 * (1 - t)

            # occasionally make a few particles brighter (flaring)
            ...
            clear_screen()
            header = (" [ NASA IMAGE DOWNLOADER - COMMAND DECK ] ").center(term_w)
            print("\n" + header + "\n")
